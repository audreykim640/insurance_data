...
bennington_vt <- places_all |>
  filter(CITY == "BENNINGTON" & STATE == "VT")

urban_two <- rbind(urban_two, bennington_vt)
```
...
write.csv(zips_rural_urban, "zips_df.csv")

# hand-enter rural zips from USPS and Google

zips_df <- read_csv("zips_df_complete.csv") |>
  rename(ZIP = `PHYSICAL ZIP`) |>
...
zips_urls <- zips_df |>
  mutate(kff_url = dbl_replace(STATE, ZIP, link = link)) |>
  filter(STATE != "PR")

```


```{r load libs for dynamic webpage}
...
get_prices_row <- function(remDr, ages, url) {
  ...
  print(urls_by_age[[1]])
  price21 <- map(urls_by_age[[1]], get_insurance_price, remDr = remDr) |>
    unlist()
  price30 <- map(urls_by_age[2], get_insurance_price, remDr = remDr) |>
     unlist()
  price40 <- map(urls_by_age[3], get_insurance_price, remDr = remDr) |>
     unlist()
  price50 <- map(urls_by_age[4], get_insurance_price, remDr = remDr) |>
    unlist()
  price60 <- map(urls_by_age[5], get_insurance_price, remDr = remDr) |>
    unlist()
  # price70 <- map(urls_by_age[6], get_insurance_price, remDr = remDr) |>
  #   unlist()

  prices_row <- cbind(price21, price30, price40, price50, price60) |>
    data.frame()
  
  colnames(prices_row) <- c("price21", "price30", "price40", "price50", "price60")
...
# isolate ny and vt; correct urls
ny_vt <- zips_urls |>
  filter(STATE == "NY" | STATE == "VT") |>
  mutate(kff_url = str_replace(kff_url, "alternate-plan-family=", "alternate-plan-family=individual"))

# isolate ks zip; correct url
ks_66547 <- zips_urls |>
  filter(ZIP == "66547") |>
  mutate(kff_url = "https://www.kff.org/interactive/subsidy-calculator/#state=ks&zip=66547&locale=Pottawatomie&income-type=dollars&income=500%2C000&people=1&alternate-plan-family=&adult-count=1&adults%5B0%5D%5Bage%5D=64&adults%5B0%5D%5Btobacco%5D=0&child-count=0")

# remove incorrect ny and vt
full_df_corrected <- full_df |>
  filter(STATE != "NY" & STATE != "VT") |>
  filter(ZIP != "66547")


prices_df_ks_66547 <- get_prices_row(remDr, ages, ks_66547$kff_url)
ks_66547_full <- cbind(ks_66547, prices_df_ks_66547)

prices_df_ny_vt <- map_dfr(ny_vt$kff_url, get_prices_row, remDr = remDr, ages = ages)
ny_vt_full <- cbind(ny_vt, prices_df_ny_vt)
full_df_corrected <- rbind(full_df_corrected, ny_vt_full, ks_66547_full)
full_df <- full_df_corrected
```
